...
%In this paper we investigate using Music Language Models (MLMs) to improve Automatic Music Transcription (AMT). The MLMs are trained on sequences of symbolic polyphonic music. We train RNN-based models as they are capable of capturing complex temporal structure present in the symbolic music data. Similar to the function of language models in ASRs, we use the MLMs to generate a prior probability for the occurrence of a sequence. We incorporate the priors into the transcription framework using Dirichlet priors. We test our hybrid models on the BACH-10 dataset and report a 2.5\% improvement in accuracy which is a significant result. 
